...
type RequireAuthz = RequireAuthorizationLayer<User, Role>;
...
async fn main() {
    ...
    let app = Router::new()
        .route(
            "/protected",
            get(protected_handler).layer(RequireAuthz::login()),
        )
        .route(
            "/protected_admin",
            get(admin_handler).layer(RequireAuthz::login_with_role(Role::Admin..)), // At least
                                                                                    // `Admin`.
        )
        .route(
            "/protected_user",
            get(user_handler).layer(RequireAuthz::login_with_role(Role::User..)), // At least
                                                                                  // `User`.
        )
